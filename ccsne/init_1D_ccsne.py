...
class InitModel(object):
        ...
        with open(outfile, 'w') as f:
            ...
            for i in range(self.nx):
                if self.mesa:
                    ...
                    f.write(f"{xzn_hse[i]} {model_state[i, self.idx['dens']]} {model_state[i, self.idx['temp']]} {model_state[i, self.idx['pres']]}  {model_state[i, self.idx['velr']]}  {model_state[i, self.idx['Ye']]} {' '.join([str(i) for i in model_state[i, self.idx['spec']:]])}\n")
    ...
    def init_1d_hse(self):
        ...
        with open('model.uniform', 'w') as f:
            ...
            for i in range(self.nx):
                f.write(f"{xzn_hse[i]} {model_hse[i, self.idx['dens']]} {model_hse[i, self.idx['temp']]} {model_hse[i, self.idx['pres']]} {model_hse[i, self.idx['velr']]}  {model_hse  [i, self.idx['Ye']]}  \n")
